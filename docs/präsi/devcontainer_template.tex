...
\begin{frame}[fragile]{Denselben Container überall ausführen}
  \textbf{Eine Container-Definition --- zwei typische Arbeitsweisen:}
  \vspace{0.4em}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{A. Alles auf dem GPU-Server}\\[2pt]
      VS Code per Remote-SSH, dort ,,Reopen in Container``.
      Code und Container leben dort, wo die GPU steht.
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{B. Code lokal, GPU remote}\\[2pt]
      Code bleibt auf dem Laptop, der Docker-Daemon auf dem Server
      übernimmt --- \hl{GPU ausleihen, ohne Code zu kopieren}.
\begin{lstlisting}[language=myshell]
...
\end{lstlisting}
  ...
  \end{columns}
  \vspace{0.2em}
  \begin{takeaway}[Kerngedanke]
    Dieselbe Umgebung auf Laptop, Workstation und geteiltem GPU-Server.
    Begleitende Host-Setups (Multi-Remote-Git, eduVPN, SMB-Mounts)
    liegen in \code{docs/help/NewPC/}.
  \end{takeaway}
\end{frame}
...
% =====================================================================
\begin{frame}{Das Template lebt von eurer Nutzung}
  Das Template ist ein \hl{Ausgangspunkt, kein fertiges Produkt}.
  Was sich im Alltag der Projekte als Reibung zeigt, gehört zurück ins Template.
  \vspace{0.6em}
  \begin{itemize}
    \item \hl{Issues im GitLab} für alles, was hakt, fehlt oder besser gehen
          könnte --- auch kleine Beobachtungen sind wertvoll.
    \item \hl{Merge Requests willkommen} für Verbesserungen am Image,
          an der Doku oder an den Begleit-Playbooks in \code{docs/help/}.
    \item \hl{Mögliche Themen}, falls jemand einen Einstieg sucht:
          Lockfile für die Python-Abhängigkeiten, CPU-Profil für Laptops
          ohne GPU, Konsolidierung der ReadMes, Härtungsprofil für
          geteilte Server.
  \end{itemize}
  \vfill
  \begin{takeaway}
    Je mehr Hände am Template, desto besser passt es auf die reale
    Arbeit der Gruppe. Schaut rein, probiert aus, schlagt vor.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Zusammenfassung}
  \begin{itemize}
    ...
    \item \hl{Offen für Beiträge} --- das Template lebt von Issues und
          Merge Requests aus den Projekten der Gruppe.
  \end{itemize}
  \vfill
  \begin{center}
