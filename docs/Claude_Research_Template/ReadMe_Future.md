# Claude Research Template — Zukunftsbild (Drei-Schienen-Architektur)

> **Status: Vision, noch nicht gebaut.** Dieses Dokument beschreibt, wohin der Workflow langfristig gehen soll: ein zusätzliches **Wiki** als Referenz-Schiene zwischen Strategie-Vault und austauschbaren Code-Repos.
> Für den heutigen Stand und die echte Anleitung: siehe [`ReadMe.md`](ReadMe.md).
> Last updated: 2026-05-07 (Inhalt unverändert; Framing 2026-06-09 nachgezogen)

> [!NOTE]
> Die Diagramme und Rituale auf dieser Seite zeigen den **Soll-Zustand**.
>
> Nicht vorhanden:
> - `og-wiki` Repo
> - `wiki-update` Skill
> - `/wiki-realign` Ritual
> - Schreib-/Lese-Verbindung zwischen DevContainer und einem Wiki-Repo
>
> Vorhanden sind heute nur die Strategie-Schiene (OgVault + Claude Desktop + MCP) und die Ausführungs-Schiene (DevContainer + Claude Code). Beide laufen isoliert. Die Brücken-Rituale aus diesem Dokument fehlen.
...
├── ReadMe.md                       ← echte README (aktueller Stand)
├── ReadMe_Future.md                ← du bist hier (Zukunftsbild, Wiki-Vision)
│
